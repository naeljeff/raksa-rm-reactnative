...
var Navbar = function (_a) {
    ...
    var handleMenuPress = function (option, idx) {
        setIndex(idx);
        onMenuChange(option);
    };
    var dataCount = react_redux_1.useSelector(function (state) { return surveySlice_1.getDataCount(state); });
    return (react_1["default"].createElement(react_native_1.View, { className: "w-full h-[50px]" },
        react_1["default"].createElement(react_native_linear_gradient_1["default"], { colors: ['#fff', '#ffbc3c', '#ffbc3c', '#ffbc3c', '#ffa008'], className: "h-full flex justify-center" },
            react_1["default"].createElement(react_native_1.View, { className: "w-full flex flex-row justify-center gap-x-2" }, menu.map(function (item, idx) { return (react_1["default"].createElement(react_native_1.View, { key: idx, className: "flex flex-row items-center justify-center" },
                react_1["default"].createElement(react_native_1.Pressable, { onPress: function () { return handleMenuPress(item, idx); }, className: "mr-2 " + (index === idx ? 'bg-[#585454] py-1.5 px-1 rounded' : '') },
                    ...
                    idx === 0 && (react_1["default"].createElement(react_native_paper_1.Badge, { size: 16, className: "absolute bg-red-500 " + (index === 0
                            ? 'top-[-3px] right-[-5px]'
                            : 'top-[-7px] right-[-8px]') }, dataCount > 99 ? '99+' : dataCount))),
